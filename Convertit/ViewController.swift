...
class ViewController: UIViewController {
    ...
    func calculateConversion() {
        guard let inputValue = Double(userInput.text!) else {
            if userInput.text != "" {
                showAlert(title: "Cannot Convert Value", message: "\"\(userInput.text!) is not a volaid number!")
            }
            return
        }
        ...
        case "liters to quarts":
            outputValue = inputValue * 1.05669
        default:
            showAlert(title: "Unexpected Error", message: "\(conversionString) cannot be identified! :(")
        }
